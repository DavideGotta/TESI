...
def ortologhi(pid: str, intergen_dir: Path, genome_ref: str) -> list[str]:
    """
    ...
    :param intergen_dir:    la directory con le sequenze intergeniche in formato fasta
...
def refined_score(motivo: Bio.motifs, pid: str, intergenica: str, entropy: list[float], intergeniche_ortologhi:dict, pwm) -> tuple:
    ...
    """
    intergeniche_ortologhi = intergeniche_ortologhi[pid]
    ...
    score_ortologhi = []
    for seq in intergeniche_ortologhi:
...
def main():
    ...
    with open("/home/davide/PycharmProjects/TESI2/diz.pickle", "rb") as f:
        diz = pickle.load(f)
    ...
    for record in SeqIO.parse(intergen, "fasta"):
        ...
        data.append(
            str(record.id) + "\t" + "\t".join(map(str, refined_score(motivo, pid, str(record.seq), entropy, diz, pwm))))
